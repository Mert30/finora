...
class UserService {
  static final CollectionReference _usersCollection = FirebaseService._firestore
      .collection('users');
...
class TransactionService {
  ...
  // ➤ GET TRANSACTIONS
  static Future<List<FirebaseTransaction>> getTransactions(
    String userId, {
    int? limit,
    DateTime? startDate,
    DateTime? endDate,
  ...
  static Stream<List<FirebaseTransaction>> getTransactionsStream(
    String userId, {
    int? limit,
    DateTime? startDate,
    DateTime? endDate,
  ...
  static Future<void> _updateUserStatsAfterTransaction(
    FirebaseTransaction transaction,
  ) async {
...
class GoalService {
  ...
  // ➤ GET GOALS
  static Future<List<FirebaseBudgetGoal>> getGoals(
    String userId, {
    bool? isActive,
    bool? isCompleted,
  }) async {
  ...
  static Future<void> updateGoal(FirebaseBudgetGoal goal) async {
    await FirebaseService._handleErrors(() async {
      await _getGoalsCollection(
        goal.userId,
      ).doc(goal.id).update(goal.toFirestore());
...
class CategoryService {
  ...
  // ➤ GET CATEGORIES
  static Future<List<FirebaseCategoryModel>> getCategories(
    String userId, {
  ...
  static Stream<List<FirebaseCategoryModel>> getCategoriesStream(
    String userId, {
    String? type,
  }) {
    Query query = _getCategoriesCollection(userId);
  ...
  static Future<void> updateCategory(FirebaseCategoryModel category) async {
    await FirebaseService._handleErrors(() async {
      await _getCategoriesCollection(
        category.userId,
      ).doc(category.id).update(category.toFirestore());
...
class CardService {
  ...
  // ➤ GET CARDS
  static Future<List<FirebaseCard>> getCards(
    String userId, {
    bool? isActive,
  }) async {
    final result = await FirebaseService._handleErrors(() async {
  ...
  static Future<void> updateCard(FirebaseCard card) async {
    await FirebaseService._handleErrors(() async {
      await _getCardsCollection(
        card.userId,
      ).doc(card.id).update(card.toFirestore());
  ...
  static Future<void> updateCardBalance(
    ...
    await FirebaseService._handleErrors(() async {
      ...
      if (userId != null) {
        ...
        final cardDoc = await FirebaseService._firestore
        ...
        if (cardDoc.docs.isNotEmpty) {
          ...
          debugPrint(
            '✅ Card balance updated (collectionGroup): $cardId -> ₺$newBalance',
          );
        } else {
  ...
  static Future<void> setDefaultCard(String userId, String cardId) async {
    await FirebaseService._handleErrors(() async {
      ...
      for (final card in allCards) {
        batch.update(_getCardsCollection(userId).doc(card.id), {
          'isDefault': false,
        });
      }

      // Set new default
      batch.update(_getCardsCollection(userId).doc(cardId), {
        'isDefault': true,
      });
